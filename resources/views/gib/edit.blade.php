    ...
    <div class="row mt-4">
        <div class="col-12 col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('gib.update', $gib->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="date" class="form-control" id="tanggal" name="tanggal"
                                    value="{{ old('tanggal', \Carbon\Carbon::parse($gib->tanggal)->format('Y-m-d')) }}"
                                    required>
                                @error('tanggal')
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" class="form-control" id="nama_donatur" name="nama_donatur"
                                    value="{{ old('nama_donatur', $gib->nama_donatur) }}" required>
                                @error('nama_donatur')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="number" class="form-control" id="nominal" name="nominal"
                                    value="{{ old('nominal', $gib->nominal) }}" min="0" step="0.01" required>
                                @error('nominal')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-2">
                                ...
                                <select id="bayar_id" name="bayar_id" class="form-control select2">
                                    @foreach ($bayar as $b)
                                        <option value="{{ $b->id }}"
                                            {{ old('bayar_id', $gib->bayar_id) == $b->id ? 'selected' : '' }}>
                                            {{ $b->cara_bayar }}
                                        </option>
                                    @endforeach
                                </select>
